...
-- for single boss boss fight, a session must be a table Session that has the following interfaces:
-- Session(boss), taking the reference to the boss and constructs a session object
-- update(), updates the session for the current frame
-- continueSession(), to check if the session has been over

---------------------------------------------------------------------------------------------------

local BossFight = require("BHElib.units.boss.boss_fight")

---@class BossFight.SingleBoss:BossFight
...
function M:update(dt)
    if not self.session then
        ...
        if index >= #classes then
            ...
            local session = SessionClass(self.boss)
